...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    const arg = argv[i];
    const next = argv[i + 1];
    if (arg === "--assessments" && next) args.assessments = path.resolve(next), i++;
    else if (arg === "--test-definitions" && next)
      args.testDefinitions = path.resolve(next), i++;
    else if (arg === "--outdir" && next) args.outdir = path.resolve(next), i++;
    else if (arg === "--help") {
      console.log(
...
function asAttempts(gameData) {
  ...
  if (gameData && Array.isArray(gameData.attempts)) return gameData.attempts.filter(Boolean);
  return [];
}
...
function gameHasLogs(attempts) {
  return attempts.some((attempt) => Array.isArray(attempt && attempt.logs) && attempt.logs.length > 0);
}
...
function reconstructStatus(test, miniGames, testConfig) {
  const totalGames = testConfig.length || Number(test.totalGames) || Object.keys(miniGames).length;
  const finalGame = testConfig.length ? testConfig[testConfig.length - 1].gameType : "";
  ...
  if (Number.isFinite(numGamesPassed) && totalGames > 0 && numGamesPassed === totalGames) {
    return {
  ...
  if (hasLogs) {
    return {
      ...
      completed_original_flag: 0,
      completed_reconstructed_flag: 0,
      completion_reason: "interaction logs exist but completion criteria not met",
      total_games: totalGames,
      final_game: finalGame,
    };
  }
...
function completionTimes(test, miniGames, status) {
  ...
  const useReconstructed = status.assessment_status === "completed_reconstructed";
  ...
  return {
    ...
    estimated_finished_at_is_reconstructed: useReconstructed && latestFinished ? 1 : 0,
    estimated_duration_ms:
      started && estimatedFinished ? Math.max(0, estimatedFinished.ms - started.ms) : "",
    estimated_duration_ms_is_reconstructed: useReconstructed && latestFinished ? 1 : 0,
  };
}
...
function buildFuzzyProfile(miniGames, testConfig, assessmentStatus) {
  const fuzzyStatus =
    assessmentStatus === "completed_original" || assessmentStatus === "completed_reconstructed"
      ? "full_assessment"
  ...
  const inferred = inferFuzzyGrade(fuzzyInputs.axes, fuzzyInputs.coverage, testConfig, true);
...
function run() {
  ...
  for (const [studentId, tests] of Object.entries(assessments || {})) {
    if (!tests || typeof tests !== "object" || String(studentId).startsWith("_")) continue;
    for (const [assessmentId, test] of Object.entries(tests)) {
      if (!test || typeof test !== "object") continue;

      const miniGames = normalizeMiniGames(test.miniGames);
      const testConfig = getTestConfig(assessmentId, testDefinitions);
      const status = reconstructStatus(test, miniGames, testConfig);
      const times = completionTimes(test, miniGames, status);
      const itemInteractions = countLogs(miniGames);
      const fuzzyProfile = buildFuzzyProfile(miniGames, testConfig, status.assessment_status);

      let weakSkillEvents = [];
      if (itemInteractions > 0) {
        weakSkillEvents = detectWeakSkills(miniGames).filter(shouldKeepWeakSkill);
        for (const weakSkill of weakSkillEvents) {
          weakRows.push(weakSkillRow(studentId, assessmentId, status.assessment_status, weakSkill));
        }
      }
      ...
      if (fuzzyProfile) {
        ...
        for (const axis of AXES) {
          const membership = (fuzzyProfile.memberships && fuzzyProfile.memberships[axis]) || {};
          fuzzyAxisRows.push({
            student_id: studentId,
            assessment_id: assessmentId,
            assessment_status: status.assessment_status,
      ...
      const baseAssessment = {
        ...
        num_games_passed: test.numGamesPassed ?? "",
        total_games: status.total_games,
        final_game: status.final_game,
        last_completed_game: test.lastCompletedGame || "",
  ...
  const { repeatedRows, maxDepth } = maxAssessmentDepthByStudent(assessmentRows);
  const fullAssessmentCount = assessmentRows.filter((row) =>
    ["completed_original", "completed_reconstructed"].includes(row.assessment_status)
  ).length;
  const itemLevelAssessmentCount = assessmentRows.filter((row) =>
    ["completed_original", "completed_reconstructed", "partial"].includes(row.assessment_status)
  ).length;

  const auditRows = [
    { metric: "total_assessments", value: assessmentRows.length },
    { metric: "completed_original", value: statusCounts.completed_original || 0 },
    { metric: "completed_reconstructed", value: statusCounts.completed_reconstructed || 0 },
    { metric: "partial", value: statusCounts.partial || 0 },
    { metric: "empty", value: statusCounts.empty || 0 },
    {
      metric: "total_item_interactions",
      value: assessmentRows.reduce((sum, row) => sum + Number(row.item_interactions || 0), 0),
    },
    { metric: "total_weak_skill_events", value: weakRows.length },
    { metric: "total_fuzzy_profiles", value: fuzzyProfileRows.length },
    { metric: "students_with_repeated_assessments", value: repeatedRows.length },
    { metric: "maximum_assessment_depth_per_student", value: maxDepth },
    { metric: "assessment_level_analysis_assessments", value: fullAssessmentCount },
    { metric: "item_level_analysis_assessments", value: itemLevelAssessmentCount },
  ];

  writeCsv(path.join(args.outdir, "assessment_reconstruction.csv"), assessmentRows, [
    "student_id",
    "assessment_id",
    "assessment_status",
  ...
  writeCsv(path.join(args.outdir, "reconstruction_audit_report.csv"), auditRows, [
    "metric",
    "value",
  ]);
  ...
  fs.writeFileSync(path.join(args.outdir, "reconstruction_audit_report.md"), markdown, "utf8");
