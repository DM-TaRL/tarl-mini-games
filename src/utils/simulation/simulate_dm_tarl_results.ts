...
function randomCoverage(): Partial<Record<keyof Axes, number>> {
  const c: Partial<Record<keyof Axes, number>> = {};
  for (const k of AXIS_KEYS) c[k] = +(0.3 + 0.7 * rnd()).toFixed(2);
  return c;
}
...
function kmeans3(data: Vec3[], k = 3, iters = 40) {
  if (data.length === 0) throw new Error("kmeans3: empty dataset");
  if (data.length < k) k = data.length;

  // Randomized, spread initialization
  const step = Math.floor(data.length / k) || 1;
  let centers: Vec3[] = Array.from(
    { length: k },
    (_, i) => data[i * step] || data[0]
  );
...
for (let i = 0; i < NUM_SESSIONS; i++) {
  ...
  const out = inferFuzzyGrade(axes, coverage, testConfig, true);
  const covMean =
...
const clusterSizes = Array.from(
  { length: K },
  (_, c) => labels.filter((x) => x === c).length
);

const centersCsvHeader =
  "cluster,center_gradeNorm,center_confidence,center_coverage,size,dominantBand,meanGradeLabel";
...
const centersCsvRows = centers.map((c, i) => {
  ...
  const dominantBand =
    Object.entries(bands).sort((a, b) => b[1] - a[1])[0]?.[0] || "N/A";
...
fs.writeFileSync(
  ...
  [centersCsvHeader, ...centersCsvRows].join("\n"),
  "utf8"
);
...
function categorize(r: Row) {
  ...
  const axesSorted = Object.entries(r).filter(([k]) =>
    AXIS_KEYS.includes(k as any)
  ) as [string, number][];
  const [minA, maxA] = [
  ...
  return {
    ...
    weakestSkill: minA[0],
    confidenceCategory:
      r.confidence < 0.6 ? "Low" : r.confidence < 0.8 ? "Medium" : "High",
    coverageCategory:
      r.coverageMean < 0.5
        ? "Partial"
        : r.coverageMean < 0.75
...
const resultsCsvHeader =
  "id,inferredGrade,confidence,coverageMean,cluster,predictedGradeLabel,proficiencyBand,dominantSkill,weakestSkill,confidenceCategory,coverageCategory," +
  AXIS_KEYS.join(",");
...
const meanStd = (vals: number[]) => {
  if (vals.length === 0) return { mean: 0, std: 0, min: 0, max: 0 };
  let sum = 0,
    sumSq = 0,
    min = Infinity,
    max = -Infinity;
...
console.log(
  `Grade norm (0–1): mean=${g.mean.toFixed(2)}, sd=${g.std.toFixed(
    2
  )}, range=${g.min.toFixed(2)}–${g.max.toFixed(2)}`
);
console.log(
  `Confidence:        mean=${conf.mean.toFixed(2)}, sd=${conf.std.toFixed(
    2
  )}, range=${conf.min.toFixed(2)}–${conf.max.toFixed(2)}`
);
console.log(
  `Coverage:          mean=${cov.mean.toFixed(2)}, sd=${cov.std.toFixed(
    2
  )}, range=${cov.min.toFixed(2)}–${cov.max.toFixed(2)}`
);
...
for (const b of Object.keys(bandCounts))
  console.log(
    `  ${b}: ${bandCounts[b]} (${((bandCounts[b] / NUM_SESSIONS) * 100).toFixed(
      1
    )}%)`
  );

/* ----------------- Proficiency summary for educators ----------------- */
const proficiencyStats: Record<
  string,
  {
    count: number;
    meanGrade: number;
    ...
    meanCoverage: number;
    dominantFreq: Record<string, number>;
    weakFreq: Record<string, number>;
  }
> = {
  Beginner: {
    count: 0,
...
const summaryHeader =
  "Band,Count,Percent,MeanGrade,MeanConfidence,MeanCoverage,TopDominantSkill,TopWeakestSkill";

for (const [band, stat] of Object.entries(proficiencyStats)) {
  if (stat.count === 0) continue;
  const topDominant =
    Object.entries(stat.dominantFreq).sort((a, b) => b[1] - a[1])[0]?.[0] ||
    "-";
  const topWeak =
    Object.entries(stat.weakFreq).sort((a, b) => b[1] - a[1])[0]?.[0] || "-";
  summaryRows.push(
    [
      band,
      stat.count,
...
fs.writeFileSync(
  "results/proficiency_summary.csv",
